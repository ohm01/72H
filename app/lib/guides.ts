...
export type GuideGroup = { id: string; label: Localized; pets?: boolean; children?: boolean; items: GuideItem[] };
...
/** Pet groups only with pets, children's groups only with children. */
export function visibleGroups(guide: Guide, household: Pick<Household, 'pets'> & { children?: number }): GuideGroup[] {
  return guide.groups.filter((g) => (!g.pets || household.pets > 0) && (!g.children || (household.children ?? 0) > 0));
}
