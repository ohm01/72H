...
- **2026-09-25**
  ...
  - Nasazení připraveno (`docs/DEPLOY.md`): `deploy/cloud-init.yaml` (ověřeno `cloud-init schema`), `scripts/create-server.sh`, `scripts/deploy.sh` (přenos otestován lokálně), `scripts/server-env.sh`, `scripts/backup-db.sh` (záloha + obnova otestovány), `scripts/fetch-cz-map.sh` (testovací výřez OK). Nový dev build s Bundle ID `com.rodinnapripravenost.app` nainstalován v simulátoru (staré `org.jennase.app72h` odinstalováno – testovací data v něm zmizela).
